...
const sensorData: SensorRow[] = [
  { no: 1, lokasi: "North Solar Grid A-12", tipe: "Photovoltaic Output", baca: "428.4 kW", status: "Optimal", tren: "up" },
  { no: 2, lokasi: "Western Water Rec. Station", tipe: "Flow Rate Monitor", baca: "12.5 L/sec", status: "Optimal", tren: "up" },
  { no: 3, lokasi: "Central Biomass Unit B-3", tipe: "Thermal Efficiency", baca: "87.2%", status: "Optimal", tren: "up" },
  { no: 4, lokasi: "East Wind Corridor T-7", tipe: "Turbine RPM Sensor", baca: "1,842 RPM", status: "Warning", tren: "down" },
  { no: 5, lokasi: "South Composting Bay C-1", tipe: "Methane Detector", baca: "2.1 ppm", status: "Optimal", tren: "stable" },
];
...
export default function Keberlanjutan() {
  ...
  return (
    <div className="flex flex-col gap-5 p-4 md:p-6 bg-gray-50/50 min-h-screen">
      {/* ── ROW 1: Three Top Cards ─────────────────────────────────── */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Green Performance */}
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col gap-4">
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 rounded-lg bg-emerald-50 text-emerald-600"><Leaf className="size-4" /></div>
            <div>
              <p className="text-[13px] font-bold text-gray-900">Green Performance</p>
              <p className="text-[11px] text-gray-400 font-medium">Q1 / January - Maret 2026</p>
            </div>
          </div>
          <div className="flex justify-center py-2">
            <CircularProgress value={94} label="Exceelent" color="#27A376" />
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col gap-4">
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 rounded-lg bg-blue-50 text-blue-600"><Droplets className="size-4" /></div>
            <div>
          ...
          <div className="flex justify-center py-2">
            <ChartContainer config={waterConfig} className="h-[120px] w-[120px]">
              <PieChart>
                <Pie data={waterData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={38} outerRadius={55} strokeWidth={2}>
                  {waterData.map((entry, i) => (<Cell key={i} fill={entry.fill} />))}
                </Pie>
          ...
          <div className="flex flex-col items-center gap-1">
            <span className="text-[11px] font-bold text-emerald-600">{pct}% Daur Ulang</span>
            <div className="flex items-center gap-4 text-[10px] text-gray-500 font-medium">
              <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-emerald-500 inline-block" />Daur ulang - 14.2k Gal</span>
              <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-blue-500 inline-block" />Sumber Segar - 6.7k Gal</span>
            </div>
          </div>
          <p className="text-[11px] text-gray-400 font-medium text-center">Distribusi air pada KST Ngijo.</p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col gap-3">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-2.5">
              <div className="p-1.5 rounded-lg bg-amber-50 text-amber-600"><Trash2 className="size-4" /></div>
              <div>
                <p className="text-[13px] font-bold text-gray-900">Metrik Limbah</p>
                <p className="text-[11px] text-gray-400 font-medium">Januari / Minggu Pertama</p>
              </div>
            </div>
          ...
          <ChartContainer config={wasteConfig} className="h-[140px] w-full">
            <BarChart data={wasteData} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
              <CartesianGrid vertical={false} strokeDasharray="3 3" stroke="#F1F5F9" />
              <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#9CA3AF" }} />
              <YAxis hide />
              <ChartTooltip content={<ChartTooltipContent />} />
              <Bar dataKey="mendatang" fill="#3B82F6" radius={[3, 3, 0, 0]} barSize={14} />
              <Bar dataKey="diproses" fill="#93C5FD" radius={[3, 3, 0, 0]} barSize={14} />
            </BarChart>
          </ChartContainer>
          <div className="flex items-center justify-center gap-4 text-[10px] text-gray-500 font-medium">
            <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-blue-500 inline-block" />Mendatang</span>
            <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-blue-300 inline-block" />Diproses</span>
          </div>
        </div>
      </div>

      {/* ── ROW 2: Energy Chart + Total Energi ────────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Dinamika Energi - spans 2 cols */}
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col gap-3">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-2.5">
              <div className="p-1.5 rounded-lg bg-blue-50 text-blue-600"><Zap className="size-4" /></div>
              <div>
          ...
          <ChartContainer config={energyConfig} className="h-[200px] w-full">
            <AreaChart data={energyData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
              ...
              <YAxis hide />
              <ChartTooltip content={<ChartTooltipContent />} />
              <Area type="monotone" dataKey="daya" stroke="#3B82F6" fill="url(#gradDaya)" strokeWidth={2.5} />
              <Area type="monotone" dataKey="konsumsi" stroke="#93C5FD" fill="url(#gradKonsumsi)" strokeWidth={2} />
            </AreaChart>
          </ChartContainer>
          <div className="flex items-center justify-center gap-4 text-[10px] text-gray-500 font-medium">
            <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-blue-500 inline-block" />Daya</span>
            <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-blue-300 inline-block" />Konsumsi</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col gap-4">
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 rounded-lg bg-amber-50 text-amber-600"><Zap className="size-4" /></div>
            <div>
              <p className="text-[13px] font-bold text-gray-900 leading-tight">Total Energi Terbarukan yang Dihasilkan</p>
              <p className="text-[11px] text-gray-400 font-medium">Q1 / January - Maret 2026</p>
            </div>
          </div>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-4xl font-extrabold text-gray-900 tracking-tight">1,284.5</span>
            <span className="text-[14px] font-semibold text-gray-500">MWh</span>
          </div>
          <div className="flex flex-col gap-3 mt-2">
            {[
              { label: "Solar Array", value: "742 MWh", color: "bg-blue-500" },
              { label: "Wind Turbines", value: "310 MWh", color: "bg-emerald-500" },
              { label: "Biomass", value: "232 MWh", color: "bg-amber-500" },
            ].map((item) => (
              <div key={item.label} className="flex items-center gap-2 text-[12px] text-gray-600 font-medium">
          ...
          <Table className="min-w-[800px]">
            <TableHeader>
              <TableRow className="bg-gray-50/80 hover:bg-gray-50/80">
                <TableHead className="font-bold text-gray-500 text-[12px] w-[60px] pl-5">No.</TableHead>
                <TableHead className="font-bold text-gray-500 text-[12px]">Lokasi Sensor</TableHead>
                <TableHead className="font-bold text-gray-500 text-[12px]">Tipe</TableHead>
                <TableHead className="font-bold text-gray-500 text-[12px]">Baca</TableHead>
                <TableHead className="font-bold text-gray-500 text-[12px]">Status</TableHead>
                <TableHead className="font-bold text-gray-500 text-[12px]">Tren</TableHead>
            ...
            <TableBody>
              {sensorData.map((row) => (
                <TableRow key={row.no} className="hover:bg-gray-50/50 group">
                  ...
                  <TableCell className="text-[13px] font-medium text-gray-900">{row.lokasi}</TableCell>
                  <TableCell className="text-[13px] text-gray-500">{row.tipe}</TableCell>
                  <TableCell className="text-[13px] text-gray-600 font-medium">{row.baca}</TableCell>
                  <TableCell>
                    <span className={cn(
                      "text-[12px] font-semibold",
                      row.status === "Optimal" ? "text-emerald-600" : "text-amber-600"
                    )}>
                      {row.status}
                    </span>
                  </TableCell>
                  <TableCell>
                    <TrendingUp className={cn(
                      "size-4",
                      row.tren === "up" ? "text-emerald-500" : row.tren === "down" ? "text-red-400 rotate-180" : "text-gray-400"
                    )} />
                  </TableCell>
                  <TableCell>
